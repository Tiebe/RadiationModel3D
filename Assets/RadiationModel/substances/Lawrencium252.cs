using System;
using System.Collections.Generic;
using RadiationModel.constants;

namespace RadiationModel.substances
{
    public class Lawrencium252 : RadioactiveSubstance
    {
        public override string name { get; } = "Lawrencium252";
        public override double halfLife { get; } = 0.33d;
        public override double atomicWeight { get; } = 252.09505d;

        public override Dictionary<double, Dictionary<double, RadioactiveSubstance>> decayProducts { get; } = new()
        {
            { 0.6d, new Dictionary<double, RadioactiveSubstance> { { 1.0d, new Mendelevium252() }, { 1.0d, new BetaParticle(1, betaPlusSpectrum) } } },
            { 0.4d, new Dictionary<double, RadioactiveSubstance> { { 1.0d, new Mendelevium248() }, { 1.0d, new AlphaParticle(10187002.09) } } },
            { 0.02d, new Dictionary<double, RadioactiveSubstance> {  } },

        }; 
        
        public static Dictionary<double, double> betaPlusSpectrum { get; } = new()
        {

        }; 
        public static Dictionary<double, double> betaMinusSpectrum { get; } = new()
        {

        }; 
    }
}
    